...
def runComsol():
    ...
    # --------------- Exporting data ------------------#
    model.result().dataset("dset1").selection().allGeom()
    model.result().export().remove("data1")
    model.result().dataset("dset1").selection().geom("geom1", 1)
    model.result().dataset("dset1").selection().geom("geom1", 1)
    model.result().dataset("dset1").selection().set(2)
    model.result().export().create("data1", "Data")
    model.result().export("data1").set("expr", "u")
    model.result().export("data1").set("unit", "m")
    model.result().export("data1").set("filename",savedirec + "\\TemperatureField.txt")

    model.result().export("data1").run()


    # --------------- Running model and saving ------------------#
